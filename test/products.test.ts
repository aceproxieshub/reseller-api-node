...
describe("client.products.list", () => {
  it("returns the unwrapped products payload", async () => {
    ...
    await expect(client.products.list()).resolves.toEqual([
      {
        durations: [
          {
            id: "duration-1",
    ...
    expect((url as URL).toString()).toBe(
      "https://reseller.example.test/api/v1/products",
    );
    expect(init?.method).toBe("GET");
  });

  it("sends the bearer token for authenticated product list requests", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
  ...
  it("throws ApiError when the products request fails", async () => {
    const fetchMock = vi.fn<typeof fetch>().mockResolvedValue(
      new Response(
        JSON.stringify({
          error: {
